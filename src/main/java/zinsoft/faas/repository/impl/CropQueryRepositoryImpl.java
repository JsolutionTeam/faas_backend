...
public class CropQueryRepositoryImpl implements CropQueryRepository {
    ...
    public void init() {
        ...
        /*fieldList.add(
            ExpressionUtils.as(
                JPAExpressions.select(code.codeVal)
                    .from(code)
                    .where(code.codeId.eq("CROP_A_CD"),
                           code.codeVal.eq(crop.cropACd)),
                "cropACdNm"));
        fieldList.add(
                ExpressionUtils.as(
                    JPAExpressions.select(code.codeVal)
                        .from(code)
                        .where(code.codeId.eq("CROP_B_CD"),
                               code.codeVal.eq(crop.cropBCd)),
                    "cropBCdNm"));
        fieldList.add(
                ExpressionUtils.as(
                    JPAExpressions.select(code.codeVal)
    ...
    public Page<CropDto> page(Map<String, Object> search, Pageable pageable) {
        ...
        JPQLQuery<CropDto> jpqQuery = query.select(allFields)
                                        .from(crop)
                                            .leftJoin(a).on(crop.cropACd.eq(a.codeVal).and(a.codeId.eq("CROP_A_CD")))
                                            .leftJoin(b).on(crop.cropBCd.eq(b.codeVal).and(b.codeId.eq("CROP_B_CD")))
                                            .leftJoin(c).on( Expressions.stringTemplate("SUBSTR({0}, 1,4)", crop.cropSeq).eq(c.codeVal).and(c.codeId.eq("CROP_C_CD")))
                                        ...
                                        .orderBy(crop.regDtm.desc(), crop.cropACd.asc(), crop.cropBCd.asc(), crop.cropSeq.asc());
...
//        }
        ...
        List<CropDto> result = jpqQuery.fetch();
